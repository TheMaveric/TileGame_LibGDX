...
public class Player extends Entity{
    ...
    { Boolean flag=false;
        ...
        if (Gdx.input.isKeyPressed(Input.Keys.W) )
        {
            flag=true;
            if(swimming==false)
                batch.draw(texture.standing.getKeyFrame(animationTime,true), pt.x,pt.y , Tile.TILEWIDTH+1,Tile.TILEHEIGHT);
            else
            if(velocity.x<0)
                batch.draw(texture.swim_left.getKeyFrame(animationTime,true), pt.x,pt.y , Tile.TILEWIDTH+1,Tile.TILEHEIGHT);
            else
        ...
        flag=true;
            ...
            if(swimming==false) {
                batch.draw(texture.particle.getKeyFrame(animationTime, true), pt.x,pt.y , Tile.TILEWIDTH+1,Tile.TILEHEIGHT);
                batch.draw(texture.attack.getKeyFrame(animationTime, true), pt.x,pt.y , Tile.TILEWIDTH+1,Tile.TILEHEIGHT);
            }
            animationTime+=deltaTime;
        }
        ...
        //bounding box
        //batch.draw(texture.boundingBox, (pt.x), pt.y, boundingBox.getWidth(),boundingBox.getHeight());
        return flag;
    }
    ...
    public void update(float delta,Batch batch) {
        ...
        if(movement(batch,delta)) {
            //save old position
            float oldX = boundingBox.x, oldY = boundingBox.y, prevVelX = velocity.x, prevVelY = velocity.y;
            collisionX = false;
            collisionY = false;
            // move on x and y
            boundingBox.setX(boundingBox.x + velocity.x * delta);
            boundingBox.setY(boundingBox.y + velocity.y * delta);
            //check collision [IMPORTANT! The rendering basically has inverted coordinates to normal XY Coordatinate system, so X is replaced with Y and vice versa and same for width and height]
    ...
    private void playerCollision(float oldX,float oldY,float prevVelX,float prevVelY)
    {
        for(int i = 0; i< World.staticEntities.size() ; i++) {
            ...
            Rectangle rectangle= new Rectangle(World.staticEntities.get(i).boundingBox.y,World.staticEntities.get(i).boundingBox.x,World.staticEntities.get(i).boundingBox.height,World.staticEntities.get(i).boundingBox.width);
            Vector2 pt=new Vector2(boundingBox.x,boundingBox.y);
            Rectangle playerBound=new Rectangle(pt.x,pt.y,boundingBox.getHeight(), boundingBox.getWidth());
            if (playerBound.x-rectangle.x<4*Tile.TILEWIDTH && playerBound.y-rectangle.y<4*Tile.TILEHEIGHT && checkCollision(rectangle)) {
                System.out.println("COLLISION");

                collisionX=rectangle.contains(playerBound.x,oldY) || rectangle.contains(playerBound.x+playerBound.width,oldY) ||
                        rectangle.contains(playerBound.x,oldY+playerBound.height) || rectangle.contains(playerBound.x+playerBound.width,oldY+playerBound.height);
                if(collisionX) {
                    boundingBox.setX(oldX);
                    //velocity.x = 0;
                }
                collisionY=rectangle.contains(oldX, playerBound.y) || rectangle.contains(oldX+playerBound.width, playerBound.y) ||
                        rectangle.contains(oldX,playerBound.y+playerBound.height) || rectangle.contains(oldX+playerBound.width,playerBound.y+playerBound.height);
                if(collisionY) {
    ...
    public boolean keyDown(int keycode) {
        switch(keycode) {
            ...
            case Input.Keys.LEFT:
                if(velocity.x>0)
                    boundingBox.setX(boundingBox.x+Tile.TILEWIDTH);
                else if(velocity.x<0)
                    boundingBox.setX(boundingBox.x-Tile.TILEWIDTH);
                if(velocity.y>0)
                    boundingBox.setY(boundingBox.y+Tile.TILEHEIGHT);
